# Making a Local LLM Actually Useful: How Llama 8B Went from 50% to 83% Accuracy for CLI Tool Selection
...
Llama 3.1 8B running locally via Ollama was the obvious candidate.

It started at roughly 50-60% accuracy. One out of every two commands was wrong.

This is the story of how it reached 83% — without changing the model weights at all. Pure infrastructure work. Part 2 covers the LoRA fine-tuning that pushed it to 90%.
...
Every component in this chain affects accuracy. Diagnosing Llama's failures revealed problems at almost every layer.
...
**Semantic matching** using embedding models was the solution:
...
Testing across 135 queries (71 multi-tenant + 64 single-tenant) showed:
...
This is where things got interesting. Llama had been treated differently from cloud models throughout the entire pipeline, and every special case was making it worse.

### Discovery: Three Hidden Code Paths

Comparing the Ollama code path against Gemini's in the batch test runner revealed three Ollama-specific branches:
...
Why did the compact prompt exist in the first place? Because it was (reasonably) assumed that Llama's 16K context window couldn't handle the full prompt.

But measuring the actual token counts told a different story:
...
Llama's tokenizer is roughly 3x more efficient than Gemini's for this type of content (bash commands, flag names, technical documentation). The full prompt fit comfortably within 16K tokens. The compact prompt was solving a problem that didn't exist.
...
Modelfile issues were also identified and fixed:
...
**1. Don't handicap small models with less context.** The instinct to "simplify for smaller models" is often counterproductive. The templates that were stripped out were the ones the small model needed *most* — it couldn't infer correct flag names from tool names alone the way a 400B model could.
...
**3. Provider-specific code paths are a maintenance trap.** Every `if ollama` branch was a place where Ollama's behavior diverged from the tested path. Bug fixes in the Gemini path didn't benefit Ollama. Unifying the code paths meant one set of tests, one set of fixes.
...
**5. The 8B ceiling is real.** Even with perfect context, 83% was the limit. The remaining failures were complex multi-parameter tools, positional arguments, and tool disambiguation — tasks that require reasoning capacity beyond what 8B parameters provide. Going further meant changing the model itself.
